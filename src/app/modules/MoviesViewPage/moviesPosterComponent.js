import axios from "axios";
import React, { useState, useEffect } from "react";

function MoviesPosterComponent(props) {
    const [moviesPoster, setmoviesPoster] = useState(null);
    //calling use effect to fetch posters from imdb-api
    // useEffect(() => {
    //     axios.get(`https://imdb-api.com/en/API/Images/k_sblaz5wr/${props.Id}/Full`).then((response)=>{
    //         console.log("res",response.data)
    //         if(response?.data?.items){
    //             setmoviesPoster(response?.data?.items)
    //         }
    //     }).catch((error)=>{
    //         console.log("error",error)
    //     })
    //   }, []);



      //main View
  return (
    <>
      <div>    
       <ul className="bg-dark">
           {props.posters ?(
               props.posters.map((mapPoster,index)=>{
                   
                       if(index<4){
                         if(index === 0){
                           return(
                             <li class="one_quarter first ">
                               {" "}
                               <div class="movie-list-item">
                                 <img
                                   class="movie-list-item-img"
                                   src={mapPoster.image}
                                   alt=""
                                   alt=""
                                 />
                               </div>
                             </li>
                           )
                         }
                       return(
                           <li class="one_quarter ">
                           <div class="movie-list-item">
                             <img
                               class="movie-list-item-img"
                               src={mapPoster.image}
                               alt=""
                               alt=" /"
                             />
               
                             {/* <button class="movie-list-item-button">view </button> */}
                           </div>
                         </li>
                       )
                   }
               })
               ):(
                poster.map((mapPoster,index)=>{
                   
                    if(index<4){
                      if(index === 0){
                        return(
                          <li class="one_quarter first ">
                            {" "}
                            <div class="movie-list-item">
                              <img
                                class="movie-list-item-img"
                                src={mapPoster.image}
                                alt=""
                                alt=""
                              />
                            </div>
                          </li>
                        )
                      }
                    return(
                        <li class="one_quarter ">
                        <div class="movie-list-item">
                          <img
                            class="movie-list-item-img"
                            src={mapPoster.image}
                            alt=""
                            alt=" /"
                          />
            
                          {/* <button class="movie-list-item-button">view </button> */}
                        </div>
                      </li>
                    )
                }
            })
               )
           }
        
      
     
       
       </ul>
     </div>
    </>
  )
}

export default MoviesPosterComponent;

const poster =[
    {
        name:'cast hero 1',
        image :'data:image/jpeg;base64,',
        asCharacter: 'tom'
    },
    {
        name:'cast hero 1',
        image :'data:image/jpeg;base64,',
        asCharacter: 'tom'
    },
    {
        name:'cast hero 1',
        image :'data:image/jpeg;base64,',
        asCharacter: 'tom'
    },
    {
        name:'cast hero 1',
        image :'data:image/jpeg;base64,',
        asCharacter: 'tom'
    },
    
 
   
]